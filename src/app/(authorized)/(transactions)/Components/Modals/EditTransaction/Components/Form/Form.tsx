...
export const Form = (): ReactElement => {
    ...
    const {updateTransaction}: ITransactionService = useTransactionService();
    const {setEditTransaction, editTransactionValue, setEditTransactionValue}:IModalActiveContext = useModalActive();

    const {comment, sum, date, category, type, _id}: ITransaction = editTransactionValue;

    const handleOnSubmitPrevent = async (e: FormEvent) => {
        e.preventDefault();
        await updateTransaction(_id, {
            comment,
            sum,
            date,
            category,
    ...
    function changeValue<K extends keyof Omit<ITransaction, "_id">>(key: K, value: ITransaction[K]): void {
        ...
        setEditTransactionValue((prev: ITransaction) => ({
            ...prev,
            [key]: value
        }));
    }
    ...
    return (
        <form id={"Update transaction form"} onSubmit={handleOnSubmitPrevent} className={cl.form}>
